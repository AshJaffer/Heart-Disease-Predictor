...
model = joblib.load("models/heart_model_v6.pkl")
scaler = joblib.load("models/scaler_v6.pkl")
encoders = joblib.load("models/label_encoders_v6.pkl")
...
alcohol_drinking = st.selectbox("Do you habitually consume alchohol?", ["No", "Yes"])
...
try:
    # Create input data with explicit type casting for numeric values
    input_data = pd.DataFrame([{**default_values, **{
        ...
        "sleep_disorder": sleep_disorder,  # Already numeric from mapping
    }}])
